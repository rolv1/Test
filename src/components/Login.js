import React, { useEffect } from 'react';
import {
    Text,
    Image,
    View,
    StyleSheet,
    TouchableWithoutFeedback,
    Button
} from "react-native"

import googleLogin from '../images/google_login.png'
import facebookLogin from '../images/facebook_login.png'
import { LoginManager, AccessToken } from 'react-native-fbsdk';

import { NavigationContainer } from '@react-navigation/native';
import { createStackNavigator } from '@react-navigation/stack';
import Movies from './Movies'
import Firebase from '../../config/Firebase'

import { connect, Provider } from 'react-redux'
import { changeUser } from '../actions/users'
import { bindActionCreators } from 'redux'
import configureStore from '../../store/configureStore'

// create our store
const store = configureStore()

import {
    GoogleSignin,
    statusCodes,
} from '@react-native-community/google-signin';


function LoginPage({ user, actions, navigation }) {
    const img = 'data:image/png;base64,'

    useEffect(() => {
        GoogleSignin.configure({
            scopes: ['https://www.googleapis.com/auth/drive.readonly'],
            webClientId: '345288558587-798iv7qo4h1p0f8povojlb5mbgjml95l.apps.googleusercontent.com',
            offlineAccess: true,
            forceCodeForRefreshToken: true,
        });
    }, [])

    /*
    Sign in with Google and save the user detail on our store
    */
    const signIn = async () => {
        try {
            await GoogleSignin.hasPlayServices();
            const user = await GoogleSignin.signIn()
                .then((user) => {
                    let details = { user }.user.user
                    actions.changeUser({
                        imgPath: details.photo,
                        username: details.givenName,
                        isLoggedIn: true
                    });
                });

        } catch (error) {
            console.log({ error })
            if (error.code === statusCodes.SIGN_IN_CANCELLED) {
                // catch :user cancelled the login flow
            } else if (error.code === statusCodes.IN_PROGRESS) {
                // catch :operation (e.g. sign in) is in progress already
            } else if (error.code === statusCodes.PLAY_SERVICES_NOT_AVAILABLE) {
                // catch :play services not available or outdated
            } else {
                //some other error happened, I won't deal with login error for now
            }
        }
    };
    /*
    Sign in with Facebook and save the user detail on our store
    */
    const signInFacebook = async () => {
        try {
            const result = await LoginManager.logInWithPermissions(['public_profile', 'email']);

            if (result.isCancelled) {
                throw 'User cancelled the login process';
            }

            // Get the users AccesToken
            const data = await AccessToken.getCurrentAccessToken();
            if (!data) {
                throw 'Something went wrong obtaining access token';
            }
            // Create a Firebase credential with the AccessToken
            const facebookCredential = Firebase.auth.FacebookAuthProvider.credential(data.accessToken);

            // Sign in the user with the credential
            const user = await Firebase.auth().signInWithCredential(facebookCredential)
                .then(
                    (curUser) => {
                        let userInfo = {
                            imgPath: curUser.user.photoURL,
                            username: curUser.additionalUserInfo.profile.first_name,
                            isLoggedIn: true
                        }
                        actions.changeUser(userInfo);

                    });
        } catch (error) {
            console.log({ error })
            // I won't deal with login errors 
        }
    }

    /*
        Since we only got two login methods this functions will check if the user
        is logged in with Google and disconnect him accordingly 
    */
    const signOut = async () => {
        GoogleSignin.isSignedIn().then(
            async (answer) => {
                if (answer) {
                    try {
                        await GoogleSignin.revokeAccess();
                        await GoogleSignin.signOut();

                        actions.changeUser({
                            imgPath: img,
                            username: 'Stranger',
                            isLoggedIn: false
                        });

                    } catch (error) {
                        console.error(error);
                    }
                }
                // Case: signed in with facebook (Not google)
                else {
                    Firebase.auth().signOut().then(function () {
                        actions.changeUser({
                            imgPath: img,
                            username: 'Stranger',
                            isLoggedIn: false
                        });

                    }).catch(function (error) {
                        console.error(error);
                    });
                }
            }
        );
    };


    return (
        <View>
            <View style={styles.container}>
                <Text style={styles.textStyle}>
                    Welcome {user.user.username}!
                </Text>
                {/* forgive me on the image white background, took it from the interet */}
                <View style={styles.imageBox}>
                    <Image source={{ uri: user.user.imgPath }} style={{ width: "100%", height: "100%", resizeMode: 'cover' }} />
                </View>

                <Text style={styles.textStyle}>
                    Please log in to continue
                </Text>

                <View style={{ display: (user.user.isLoggedIn ? 'flex' : 'none'), marginTop: '2%' }}>
                    <Button
                        title={"רשימת סרטים"}
                        color={'green'}
                        onPress={() => {
                            navigation.navigate('Movies')
                        }}
                    />
                </View>
            </View>


            <View style={styles.footer}>
                <TouchableWithoutFeedback
                    onPress={signIn}
                >
                    <View
                        style={{ flex: 5, marginRight: '2%', display: (user.user.isLoggedIn ? 'none' : 'flex') }}
                    >
                        <Image source={googleLogin} style={{ flex: 1, width: null, height: null, resizeMode: 'contain' }} />
                    </View>
                </TouchableWithoutFeedback>

                <TouchableWithoutFeedback
                    onPress={signInFacebook}
                >
                    <View
                        style={{ flex: 6, display: (user.user.isLoggedIn ? 'none' : 'flex') }}
                    >
                        <Image source={facebookLogin} style={{ flex: 1, width: null, height: null, resizeMode: 'contain' }} />

                    </View>
                </TouchableWithoutFeedback>

                <View style={{ display: (user.user.isLoggedIn ? 'flex' : 'none'), alignSelf: 'center', marginLeft: '40%' }}>
                    <Button
                        title={"log out"}
                        color={'green'}
                        onPress={signOut}
                    />
                </View>
            </View>

        </View>
    );

}

// Creating the navigator
const Stack = createStackNavigator();
function Login() {
    return (
        <Provider store={store}>
            <NavigationContainer>
                <Stack.Navigator initialRouteName="LoginP" >
                    <Stack.Screen options={{ headerShown: false }} name="LoginP" component={LoginPageContainer} />
                    <Stack.Screen options={{ headerShown: false }} name="Movies" component={MovieContainer} />

                </Stack.Navigator>
            </NavigationContainer>
        </Provider>

    );
}

const styles = StyleSheet.create({

    container: {
        fontStyle: 'italic',
        fontWeight: 'bold',
        height: '80%',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center'
    },
    imageBox: {
        height: '25%',
        width: '40%',
        margin: '3%'
    },
    footer: {
        height: '20%',
        width: '90%',
        alignSelf: 'center',
        display: 'flex',
        flexDirection: 'row',
        justifyContent: 'space-between'
    },

    textStyle: {
        fontSize: 20
    }
});


// Pick the props that we will use
const mapStateToProps = state => ({
    user: state.user,
});


// Pick the actions that we will use
const ActionCreators = Object.assign(
    {},
    { changeUser } ,
);

const mapDispatchToProps = dispatch => ({
    actions: bindActionCreators(ActionCreators, dispatch),
});
// Connect the screens to Redux
let LoginPageContainer = connect(mapStateToProps, mapDispatchToProps)(LoginPage)
let MovieContainer = connect(mapStateToProps, mapDispatchToProps)(Movies)


export default Login



